...
class DeleteBotCommand extends Command
{
    ...
    public function handle(): void
    {
        ...
        $path = base_path('app/Http/Resources/' . $model . 'DetailResource.php');
        $this->deleteFile($path);
        ...
        $path = base_path('resources/views/admin/pages/' . $model);
        $this->deleteFolder($path);

        // feature test
        $path = base_path('tests/Feature/Http/Controllers/Api/V1/' . $model);
        $this->deleteFolder($path);

        // advanced search Driver and Handler
        $path = base_path('app/Services/AdvancedSearchFields/Drivers/' . $model . 'Driver.php');
        $this->deleteFile($path);

        $path = base_path('app/Services/AdvancedSearchFields/Handlers/' . $model . 'Handler.php');
        $this->deleteFile($path);
