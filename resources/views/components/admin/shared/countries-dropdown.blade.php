...
@push('js')
    <script>
        var country_id_dropdown = $('#{{$id}}').select2({
            placeholder: '{{__('core.search_country')}}',
            allowClear: true,
            ajax: {
                url: '{{route('admin.select.country')}}',
                methods: 'POST',
                dataType: 'json',
        ...
        var selectedRows = @json($selectedRows);
            selectedRows.forEach(function (e) {
                country_id_dropdown.append(new Option(e.value, e.uuid, false, false));
            });
        country_id_dropdown.val(ids).trigger('change');
        @endif
