...
@push('js')
    <script>
        var user_id_dropdown = $('#{{$id}}').select2({
            placeholder: '{{__('core.search_user')}}',
            allowClear: true,
            ajax: {
                url: '{{route('admin.select.user')}}',
                methods: 'POST',
                dataType: 'json',
        ...
        selectedRows.forEach(function (e) {
            user_id_dropdown.append(new Option(e.value, e.id, false, false));
        });
        user_id_dropdown.val(ids).trigger('change');
        @endif
