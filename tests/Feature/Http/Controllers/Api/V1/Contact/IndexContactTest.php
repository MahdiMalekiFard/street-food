...
beforeEach(function () {
    ...
    foreach ($items as $item) {
        $model = Contact::factory(1)->create([
...
it('index contact', function (array $payload = [], array $expected = []) {
    $query = Arr::query([
        ...[
            'page'       => 1,
            'page_limit' => 5,
    ...
    $response = login()->getJson('api/v1/contact?' . $query);
    ...
    if ($assertJsonItem = Arr::get($expected, 'assertJsonItem')) {
        expect()->assertJsonItem($result['data'], $assertJsonItem, 'Contact with the expected structure is not returned');
    }
})->with([
