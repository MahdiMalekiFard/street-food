...
use App\Services\Permissions\Models\ContactPermissions;
...
it('delete contact', function (array $payload = [], array $expected = []) {
    $model = Contact::factory()->create([
        'id' => 1,
    ]);

    $response = login(permissions: Arr::get($payload, 'permissions', [SharedPermissions::Admin]))->deleteJson('api/v1/contact/' . Arr::get($payload, 'id', 1));
    ...
    if (Arr::get($expected, 'status') === 200 && Schema::hasColumn('contacts', 'deleted_at')) {
        assertDatabaseHas('contacts', [
            'id' => 1,
        ]);
        assertNotNull($model->deleted_at, "contact field deleted_at is null");
    ...
    'have permission'        => [
        'payload'  => ['permissions' => [contactPermissions::Delete]],
        'expected' => ['status' => 200, 'assertJsonStructure' => [], 'assertJson' => ['data' => true]],
    ],
