...
beforeEach(function () {
    ...
    foreach ($items as $item) {
        $model = ArtGallery::factory(1)->create([
...
it('index artGallery', function (array $payload = [], array $expected = []) {
    $query = Arr::query([
        ...[
            'page'       => 1,
            'page_limit' => 5,
    ...
    $response = login()->getJson('api/v1/art-gallery?' . $query);
    ...
    if ($assertJsonItem = Arr::get($expected, 'assertJsonItem')) {
        expect()->assertJsonItem($result['data'], $assertJsonItem, 'ArtGallery with the expected structure is not returned');
    }
})->with([
