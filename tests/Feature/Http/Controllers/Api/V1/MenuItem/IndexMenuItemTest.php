...
beforeEach(function () {
    ...
    foreach ($items as $item) {
        $model = MenuItem::factory(1)->create([
...
it('index menuItem', function (array $payload = [], array $expected = []) {
    $query = Arr::query([
        ...[
            'page'       => 1,
            'page_limit' => 5,
    ...
    $response = login()->getJson('api/v1/menu-item?' . $query);
    ...
    if ($assertJsonItem = Arr::get($expected, 'assertJsonItem')) {
        expect()->assertJsonItem($result['data'], $assertJsonItem, 'MenuItem with the expected structure is not returned');
    }
})->with([
