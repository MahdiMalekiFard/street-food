...
beforeEach(function () {
    ...
    foreach ($items as $item) {
        $model = Opinion::factory(1)->create([
...
it('index opinion', function (array $payload = [], array $expected = []) {
    $query = Arr::query([
        ...[
            'page'       => 1,
            'page_limit' => 5,
    ...
    $response = login()->getJson('api/v1/opinion?' . $query);
    ...
    if ($assertJsonItem = Arr::get($expected, 'assertJsonItem')) {
        expect()->assertJsonItem($result['data'], $assertJsonItem, 'Opinion with the expected structure is not returned');
    }
})->with([
