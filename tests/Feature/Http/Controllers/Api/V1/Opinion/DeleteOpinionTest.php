...
it('delete opinion', function (array $payload = [], array $expected = []) {
    $model = Opinion::factory()->create([
        'id' => 1,
    ]);

    $response = login(permissions: Arr::get($payload, 'permissions', [SharedPermissions::Admin]))->deleteJson('api/v1/opinion/' . Arr::get($payload, 'id', 1));
    ...
    if (Arr::get($expected, 'status') === 200 && Schema::hasColumn('opinions', 'deleted_at')) {
        assertDatabaseHas('opinions', [
            'id' => 1,
        ]);
        assertNotNull($model->deleted_at, "opinion field deleted_at is null");
    } elseif (Arr::get($expected, 'status') === 200) {
        assertDatabaseMissing('opinions', [
            'id' => 1,
        ]);
    } else {
        assertDatabaseHas('opinions', [
    ...
    'have permission'        => [
        'payload'  => ['permissions' => [opinionPermissions::Delete]],
        'expected' => ['status' => 200, 'assertJsonStructure' => [], 'assertJson' => ['data' => true]],
    ],
