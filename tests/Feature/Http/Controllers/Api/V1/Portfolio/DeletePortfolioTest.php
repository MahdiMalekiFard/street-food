...
uses()->group('portfolio');

it('delete portfolio', function (array $payload = [], array $expected = []) {
    $model = Portfolio::factory()->create([
        'id' => 1,
    ]);

    $response = login(permissions: Arr::get($payload, 'permissions', [SharedPermissions::Admin]))->deleteJson('api/v1/portfolio/' . Arr::get($payload, 'id', 1));
    ...
    if (Arr::get($expected, 'status') === 200 && Schema::hasColumn('portfolios', 'deleted_at')) {
        assertDatabaseHas('portfolios', [
            'id' => 1,
        ]);
        assertNotNull($model->deleted_at, "portfolio field deleted_at is null");
    } elseif (Arr::get($expected, 'status') === 200) {
        assertDatabaseMissing('portfolios', [
            'id' => 1,
        ]);
    } else {
        assertDatabaseHas('portfolios', [
    ...
    'have permission'        => [
        'payload'  => ['permissions' => [portfolioPermissions::Delete]],
        'expected' => ['status' => 200, 'assertJsonStructure' => [], 'assertJson' => ['data' => true]],
    ],
