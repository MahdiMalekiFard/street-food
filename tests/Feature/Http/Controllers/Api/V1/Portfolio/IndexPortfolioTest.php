...
beforeEach(function () {
    ...
    foreach ($items as $item) {
        $model = Portfolio::factory(1)->create([
...
it('index portfolio', function (array $payload = [], array $expected = []) {
    $query = Arr::query([
        ...[
            'page'       => 1,
            'page_limit' => 5,
    ...
    $response = login()->getJson('api/v1/portfolio?' . $query);
    ...
    if ($assertJsonItem = Arr::get($expected, 'assertJsonItem')) {
        expect()->assertJsonItem($result['data'], $assertJsonItem, 'Portfolio with the expected structure is not returned');
    }
})->with([
