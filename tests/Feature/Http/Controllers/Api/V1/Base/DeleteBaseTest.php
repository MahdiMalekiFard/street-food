...
it('delete base', function (array $payload = [], array $expected = []) {
    $model = Base::factory()->create([
        'id' => 1,
    ]);

    $response = login(permissions: Arr::get($payload, 'permissions', [SharedPermissions::Admin]))->deleteJson('api/v1/base/' . Arr::get($payload, 'id', 1));
    ...
    if (Arr::get($expected, 'status') === 200 && Schema::hasColumn('bases', 'deleted_at')) {
        assertDatabaseHas('bases', [
            'id' => 1,
        ]);
        assertNotNull($model->deleted_at, "base field deleted_at is null");
    } elseif (Arr::get($expected, 'status') === 200) {
        assertDatabaseMissing('bases', [
            'id' => 1,
        ]);
    } else {
        assertDatabaseHas('bases', [
    ...
    'have permission'        => [
        'payload'  => ['permissions' => [basePermissions::Delete]],
        'expected' => ['status' => 200, 'assertJsonStructure' => [], 'assertJson' => ['data' => true]],
    ],
