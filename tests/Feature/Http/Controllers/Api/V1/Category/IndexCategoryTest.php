...
beforeEach(function () {
    ...
    foreach ($items as $item) {
        $model = Category::factory(1)->create([
...
it('index category', function (array $payload = [], array $expected = []) {
    $query = Arr::query([
        ...[
            'page'       => 1,
            'page_limit' => 5,
    ...
    $response = login()->getJson('api/v1/category?' . $query);
    ...
    if ($assertJsonItem = Arr::get($expected, 'assertJsonItem')) {
        expect()->assertJsonItem($result['data'], $assertJsonItem, 'Category with the expected structure is not returned');
    }
})->with([
