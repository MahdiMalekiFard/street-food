...
it('show area', function (array $payload = [], array $expected = []) {
    Area::factory()->create([
        'id' => 1,
    ]);

    $response = login(permissions: Arr::get($payload, 'permissions', [SharedPermissions::Admin]))->getJson('api/v1/area/' . Arr::get($payload, 'id', 1));
    ...
    'have permission'        => [
        'payload'  => ['permissions' => [AreaPermissions::Show]],
        'expected' => ['status' => 200, 'assertJsonStructure' => ['id', 'created_at', 'updated_at'], 'assertJson' => ['data' => ['id' => 1]]],
    ],
