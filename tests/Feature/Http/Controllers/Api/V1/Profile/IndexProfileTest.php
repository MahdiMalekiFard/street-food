...
beforeEach(function () {
    ...
    foreach ($items as $item) {
        $model = Profile::factory(1)->create([
...
it('index profile', function (array $payload = [], array $expected = []) {
    $query = Arr::query([
        ...[
            'page'       => 1,
            'page_limit' => 5,
    ...
    $response = login()->getJson('api/v1/profile?' . $query);
    ...
    if ($assertJsonItem = Arr::get($expected, 'assertJsonItem')) {
        expect()->assertJsonItem($result['data'], $assertJsonItem, 'Profile with the expected structure is not returned');
    }
})->with([
