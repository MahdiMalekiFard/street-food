...
use App\Models\Menu;
use App\Services\Permissions\Models\MenuPermissions;
...
it('delete menu', function (array $payload = [], array $expected = []) {
    $model = Menu::factory()->create([
        'id' => 1,
    ]);

    $response = login(permissions: Arr::get($payload, 'permissions', [SharedPermissions::Admin]))->deleteJson('api/v1/menu/' . Arr::get($payload, 'id', 1));
    ...
    if (Arr::get($expected, 'status') === 200 && Schema::hasColumn('menus', 'deleted_at')) {
        assertDatabaseHas('menus', [
            'id' => 1,
        ]);
        assertNotNull($model->deleted_at, "menu field deleted_at is null");
    } elseif (Arr::get($expected, 'status') === 200) {
        assertDatabaseMissing('menus', [
            'id' => 1,
        ]);
    } else {
        assertDatabaseHas('menus', [
    ...
    'have permission'        => [
        'payload'  => ['permissions' => [menuPermissions::Delete]],
        'expected' => ['status' => 200, 'assertJsonStructure' => [], 'assertJson' => ['data' => true]],
    ],
