...
it('show locality', function (array $payload = [], array $expected = []) {
    Locality::factory()->create([
        'id' => 1,
    ]);

    $response = login(permissions: Arr::get($payload, 'permissions', [SharedPermissions::Admin]))->getJson('api/v1/locality/' . Arr::get($payload, 'id', 1));
    ...
    'have permission'        => [
        'payload'  => ['permissions' => [LocalityPermissions::Show]],
        'expected' => ['status' => 200, 'assertJsonStructure' => ['id', 'created_at', 'updated_at'], 'assertJson' => ['data' => ['id' => 1]]],
    ],
