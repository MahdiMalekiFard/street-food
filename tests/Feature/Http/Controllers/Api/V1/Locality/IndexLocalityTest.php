...
beforeEach(function () {
    ...
    foreach ($items as $item) {
        $model = Locality::factory(1)->create([
...
it('index locality', function (array $payload = [], array $expected = []) {
    $query = Arr::query([
        ...[
            'page'       => 1,
            'page_limit' => 5,
    ...
    $response = login()->getJson('api/v1/locality?' . $query);
    ...
    if ($assertJsonItem = Arr::get($expected, 'assertJsonItem')) {
        expect()->assertJsonItem($result['data'], $assertJsonItem, 'Locality with the expected structure is not returned');
    }
})->with([
