...
beforeEach(function () {
    ...
    foreach ($items as $item) {
        $model = ActivationCode::factory(1)->create([
...
it('index activationCode', function (array $payload = [], array $expected = []) {
    $query = Arr::query([
        ...[
            'page'       => 1,
            'page_limit' => 5,
    ...
    $response = login()->getJson('api/v1/activation-code?' . $query);
    ...
    if ($assertJsonItem = Arr::get($expected, 'assertJsonItem')) {
        expect()->assertJsonItem($result['data'], $assertJsonItem, 'ActivationCode with the expected structure is not returned');
    }
})->with([
