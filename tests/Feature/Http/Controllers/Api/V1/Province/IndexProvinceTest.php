...
beforeEach(function () {
    ...
    foreach ($items as $item) {
        $model = Province::factory(1)->create([
...
it('index province', function (array $payload = [], array $expected = []) {
    $query = Arr::query([
        ...[
            'page'       => 1,
            'page_limit' => 5,
    ...
    $response = login()->getJson('api/v1/province?' . $query);
    ...
    if ($assertJsonItem = Arr::get($expected, 'assertJsonItem')) {
        expect()->assertJsonItem($result['data'], $assertJsonItem, 'Province with the expected structure is not returned');
    }
})->with([
