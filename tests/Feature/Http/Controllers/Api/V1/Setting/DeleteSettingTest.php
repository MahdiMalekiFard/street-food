...
it('delete setting', function (array $payload = [], array $expected = []) {
    $model = Setting::factory()->create([
        'id' => 1,
    ]);

    $response = login(permissions: Arr::get($payload, 'permissions', [SharedPermissions::Admin]))->deleteJson('api/v1/setting/' . Arr::get($payload, 'id', 1));
    ...
    if (Arr::get($expected, 'status') === 200 && Schema::hasColumn('settings', 'deleted_at')) {
        assertDatabaseHas('settings', [
            'id' => 1,
        ]);
        assertNotNull($model->deleted_at, "setting field deleted_at is null");
    ...
    'have permission'        => [
        'payload'  => ['permissions' => [settingPermissions::Delete]],
        'expected' => ['status' => 200, 'assertJsonStructure' => [], 'assertJson' => ['data' => true]],
    ],
