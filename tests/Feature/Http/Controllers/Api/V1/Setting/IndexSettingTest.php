...
beforeEach(function () {
    ...
    foreach ($items as $item) {
        $model = Setting::factory(1)->create([
...
it('index setting', function (array $payload = [], array $expected = []) {
    $query = Arr::query([
        ...[
            'page'       => 1,
            'page_limit' => 5,
    ...
    $response = login()->getJson('api/v1/setting?' . $query);
    ...
    if ($assertJsonItem = Arr::get($expected, 'assertJsonItem')) {
        expect()->assertJsonItem($result['data'], $assertJsonItem, 'Setting with the expected structure is not returned');
    }
})->with([
